...
const AlreadyBought = () => {
  ...
  const fetchLesson = async () => {
    ...
    const res = await fetch(
      `/api/LessonPack/findLessonPack?email=${lessonValue}`
    );
  ...
  return (
    <div>
      <Head>
        <title>Already Bought Lessons Pack - Punjabi Lesson</title>
        <meta name="viewport" content="initial-scale=1.0, width=device-width" />
        <meta
          name="description"
          content="Access the lessons you've purchased with your lessons pack. Learn at your own pace, revisit your lessons, and make the most of your educational investment."
        />
      </Head>
        ...
        {resData?.lesson.length != 0 ? (
          resData?.lesson.map((slot, index) => (
            ...
            >
              ...
              <div className="mt-4 break-words rounded-2xl grid  grid-cols-1 sm:grid-cols-2  justify-center  items-center gap-4 text-lg">
                ...
                <p>
                  <span className="font-bold">Amount: </span>${slot.amount}
                </p>
                <p>
              ...
              <div className="flex justify-center  items-center mt-4">
                {slot.lessons.length == 5 ? (
                  <div className="flex  flex-col gap-2">
                    <p className="text-xl font-bold">
                      You attented your all lessons
                    </p>{" "}
                    <Link
                      href={{
          ...
          <div>
            <h2 className="text-4xl mx-4 text-center mb-16 text-black">
              There is no lesson pack that has been bought with this email.
            </h2>
          </div>
        )}
      </div>
